...
def cmd_api_client(message):
    if not api_key_file.exists():
        ...
        r = requests.post('http://0.0.0.0:5000/api/key', json={'username':username, 'password':password})
    ...
    r = requests.post('http://0.0.0.0:5000/api/post', json={'text':message}, headers={'X-APIKEY': api_key})
    print(r.text)
